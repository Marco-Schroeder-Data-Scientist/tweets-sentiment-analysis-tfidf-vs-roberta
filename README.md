...
|---|---|---|
| TF-IDF + Logistic Regression | **~71%** | Mejor modelo — balance sólido precision/recall |
| TF-IDF + Random Forest | **~70%** | Resultado competitivo |
| TF-IDF + BernoulliNB | ~68% | Baseline clásico |
| RoBERTa (twitter-latest) | ~40% | Colapsa prediciendo casi todo como negativo |
...
1. Ejecutar las celdas del notebook `analisis_sentimientos.ipynb` en orden
2. El pipeline entrena los modelos clásicos con GridSearchCV
3. El benchmark RoBERTa se ejecuta al final (puede tardar 5–15 min en CPU)
4. Las métricas y matrices de confusión se generan automáticamente al final
